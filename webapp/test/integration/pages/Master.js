		...
		onTheMasterPage : {
			...
			actions : {
				...
				iSortTheListOnName : function () {
					return this.iChooseASorter("sortButton", "Sort By <Status>");
				},
						...
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("TransportationNum") === oObject.TransportationNum;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].TransportationNum;
									} else {
										sCurrentId = aItemsNotInTheList[0].TransportationNum;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("Transportations", {
										TransportationNum : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("TransportationNum"),
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						...
						id: "masterPageTitle",
						autoWait: false,
						matchers: new PropertyStrictEquals({name : "text", value : "<Transportations> (0)"}),
						success: function () {
				...
				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("Status");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all Transportations to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "masterPageTitle",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "text", value : "<Transportations> (" + iExpectedLength + ")"}),
								success : function () {
